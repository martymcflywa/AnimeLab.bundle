...
# that will be described below. I am using a function from below that pulls the thumb from the head of the page
    oc.add(DirectoryObject(key=Callback(ShowRSS, title="RSS Shows", url='http://www.webpage1.com'),
                           title="RSS Shows", thumb=GetThumb(url='http://www.webpage1.com')))
    oc.add(DirectoryObject(key=Callback(ShowHTML, title="HTML Page Source Shows", url='http://www.webpage1.com'),
                           title="HTML Page Source Shows", thumb=GetThumb(url='http://www.webpage2.com')))
    oc.add(DirectoryObject(key=Callback(ShowJSON, title="JSON Source Shows", url='http://www.webpage1.com'),
                           title="JSON Source Shows", thumb=GetThumb(url='http://www.webpage3.com')))
...
def ShowJSON(title):
    ...
    json_url = 'http://www.hulu.com/mozart/v1.h2o/shows/' + show_id + '/episodes?free_only=0&show_id=' + show_id + \
        '&sort=seasons_and_release&video_type=episode&items_per_page=32&access_token=Jk0-QLbtXxtSMOV4TUnwSXqhhDM%3DaoY_yiNlac0ed1d501bee91624b25159a0c57b05d5f34fa3dc981da5c5e9169daf661ffb043b2805717849b7bdeb8e01baccc43f'
